...
func generateNonStandardCipher() TestKeystoreInfo {
	...
	privateKeyHex := "3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4"
	privateKeyBytes, err := hex.DecodeString(privateKeyHex)
	if err != nil {
		log.Fatalf("Failed to decode private key: %v", err)
	}
...
func updateDocumentation(newKeystores []TestKeystoreInfo) {
	...
	if err != nil {
		...
		readmeContent = []byte("# Test Keystore Files\n\nThis directory contains sample keystore files for testing the keystore validation and import functionality.\n\n")
	}

	// Add information about the new keystores
	additionalContent := "\n## Additional Test Keystore Files\n\n"

	// Add valid keystores
	additionalContent += "### Additional Valid Keystore Files\n\n"
	for _, ks := range newKeystores {
		if ks.Valid {
			additionalContent += fmt.Sprintf("- `%s`: %s\n", ks.Filename, ks.Notes)
			additionalContent += fmt.Sprintf("  - Password: `%s`\n", ks.Password)
			additionalContent += fmt.Sprintf("  - Address: `%s`\n", ks.Address)
			additionalContent += fmt.Sprintf("  - KDF: %s\n\n", ks.KDF)
		}
	}

	// Add invalid keystores
	additionalContent += "### Additional Invalid Keystore Files\n\n"
	for _, ks := range newKeystores {
		if !ks.Valid {
			additionalContent += fmt.Sprintf("- `%s`: %s\n", ks.Filename, ks.Notes)
		}
	...
	updatedReadme := string(readmeContent) + additionalContent
	err = ioutil.WriteFile(readmePath, []byte(updatedReadme), 0644)
	if err != nil {
		log.Fatalf("Failed to update README.md: %v", err)
	}

	fmt.Println("Updated README.md with information about the new keystore files")
}
